...
def load_and_process_data(args):
    ...
    benchmark_types = [d for d in os.listdir(benchmark_root_dir)
    ...
    for benchmark_type in benchmark_types:
        aggregated_scores[benchmark_type] = {metric: {method: [] for method in METHODS} for metric in METRICS}
        benchmark_type_path = os.path.join(benchmark_root_dir, benchmark_type)
        
        sample_folders = [d for d in os.listdir(benchmark_type_path)
                        ...
                        if method_name in METHODS:
                            for metric_name, score_value in method_scores.items():
                                if metric_name in METRICS:
                                    aggregated_scores[benchmark_type][metric_name][method_name].append(score_value)
                                else:
                                    print(f"Warning: Unknown metric '{metric_name}' in method '{method_name}' in file {json_file_path}")
                        # else:
    ...
    for benchmark_type, metrics_data in aggregated_scores.items():
        ...
        for metric_name, methods_data in metrics_data.items():
            ...
            for method_name, scores_list in methods_data.items():
                if scores_list:
                    average_scores[benchmark_type][metric_name][method_name] = statistics.mean(scores_list)
                else:
                    raise ValueError(f"No scores found for {method_name} in {metric_name} for {benchmark_type}")
...
def plot_histograms(average_scores, cli_args_for_title):
    ...
    for benchmark_type, metrics_data in average_scores.items():
        for metric_name, methods_data in metrics_data.items():
            ...
            if not method_names or not any(avg_values): # if all avg_values are 0 or method_names is empty
                raise ValueError(f"No valid data to plot for {benchmark_type} - {metric_name}")


            plt.figure(figsize=(10, 6))
            bars = plt.bar(method_names, avg_values, color=['skyblue', 'lightcoral', 'lightgreen', 'gold'])

            plt.xlabel("Method")
            plt.ylabel(f"Average {metric_name}")
            plt.title(f"Average {metric_name} for {benchmark_type}\nConfig: {config_str}", fontsize=10)
            ...
            for bar in bars:
                yval = bar.get_height()
                plt.text(bar.get_x() + bar.get_width()/2.0, yval, f'{yval:.3f}', va='bottom', ha='center') # Adjust format as needed
...
def main():
    parser = argparse.ArgumentParser(description="Generate histograms from benchmark metrics.")
    ...
    parser.add_argument('--inject-k', action='store_true', help='K injection was enabled')
    parser.add_argument('--inject-q', action='store_true', help='Q injection was enabled')
    parser.add_argument('--inject-v', action='store_true', help='V injection was enabled')
    parser.add_argument('--layers', type=str, default='vital', choices=['all', 'vital'],
