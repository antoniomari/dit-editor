    ...
    if isinstance(pipe.pipe, EditedFluxPipeline):
        _ = pipe.run(
            ...
            latents=image2latent(
                pipe.pipe, image, latent_nudging_scalar=latent_nudging_scalar
            ),
...
def resize_bounding_box(
    ...
    out_mask = torch.zeros(
        (w_target, h_target), dtype=bb_mask.dtype, device=bb_mask.device
    )
...
def place_image_in_bounding_box(
    image_tensor_whc: torch.Tensor, mask_tensor_wh: torch.Tensor
    ...
    if not (
        image_tensor_whc.ndim == 3
        and image_tensor_whc.shape[0] > 0
        and image_tensor_whc.shape[1] > 0
    ):
        raise ValueError(
    ...
    empty_output_image = torch.zeros(
        ...
        canvas_h,
        num_channels,
        dtype=image_tensor_whc.dtype,
        device=image_tensor_whc.device,
    )
    empty_new_mask = torch.zeros(
        canvas_w, canvas_h, dtype=mask_tensor_wh.dtype, device=mask_tensor_wh.device
    )
    ...
    try:
        ...
        resized_image_chw = TF.resize(
            image_tensor_chw, [resized_img_h, resized_img_w], antialias=True
        )
    ...
    output_image_whc[
        paste_x_start:paste_x_end, paste_y_start:paste_y_end, :
    ] = resized_image_whc
...
def compose_noise_masks(
    ...
    PATCH_SIZE = 16
    if (
        background_image.size[0] % PATCH_SIZE != 0
        or background_image.size[1] % PATCH_SIZE != 0
    ):
        raise ValueError(
    ...
    target_size = (
        background_image.size[1] // PATCH_SIZE,
        background_image.size[0] // PATCH_SIZE,
    )

    # process the options
    if option == "bg":
        # only background noise
        bg_noise = get_inverted_input_noise(
            cached_pipe, background_image, num_steps=num_inversion_steps
        )
        ...
        # print("Placed Mask")
        resized_mask_img = Image.fromarray(
            (resized_mask.numpy() * 255).astype(np.uint8)
        )
        ...
        if photoshop_fg_noise:
            ...
            photoshop_img = Image.fromarray(
                (
                    ...
                    * ~resized_mask.cpu().unsqueeze(-1)
                    + torch.tensor(np.array(reframed_fg_img))
                    * resized_mask.cpu().unsqueeze(-1)
                ).numpy()
            )
            # display(photoshop_img)
            fg_noise = get_inverted_input_noise(
                cached_pipe, photoshop_img, num_steps=num_inversion_steps
            )
        else:
            fg_noise = get_inverted_input_noise(
                cached_pipe, reframed_fg_img, num_steps=num_inversion_steps
            )
        bg_noise = get_inverted_input_noise(
            cached_pipe, background_image, num_steps=num_inversion_steps
        )
        ...
        segmented_fg_image = torch.tensor(
            np.array(foreground_mask.resize(foreground_image.size))
        ).to(torch.bool).unsqueeze(-1) * torch.tensor(np.array(foreground_image))
        ...
        foreground_mask = foreground_mask.convert(
            "RGB"
        )  # to avoid extraction of contours and make work with function
        ...
        bg_noise_init = (
            bg_noise[-1].squeeze(0) if isinstance(bg_noise, list) else bg_noise
        )
        fg_noise_init = (
            fg_noise[-1].squeeze(0) if isinstance(fg_noise, list) else fg_noise
        )
        ...
        reframed_segmentation_mask = torch.from_numpy(reframed_segmentation_mask).to(
            dtype=bool
        )
        latent_mask = (
            resize_bounding_box(
    ...
    latent_bbox_mask = (
        resize_bounding_box(
            torch.from_numpy(
                np.array(target_mask.resize(background_image.size))
            ),  # reseize just to be sure
            target_size=target_size,
        )
        .flatten()
        .unsqueeze(-1)
