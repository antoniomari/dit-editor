...
from dit_edit.core.activation_cache import (
    ActivationCacheHandler,
    FluxActivationCache,
    ModelActivationCache,
)
from dit_edit.core.flux_pipeline import EditedFluxPipeline
from dit_edit.core.hooks import edit_streams_hook, locate_block, register_general_hook
from dit_edit.core.processors.cached_flux_attn_processor_3 import (
    CachedFluxAttnProcessor3_0,
)
from dit_edit.core.qkv_cache.cache import QKVCache
from dit_edit.core.qkv_cache.handler import QKVCacheFluxHandler
from dit_edit.core.scheduler_inversion import (
    FlowMatchEulerDiscreteSchedulerForInversion,
)
...
class CachedPipeline:
    ...
    def setup_cache(
        ...
        if use_activation_cache:
            if isinstance(self.pipe, EditedFluxPipeline) or isinstance(
                self.pipe, FluxPipeline
            ):
                activation_cache = FluxActivationCache()
            else:
                raise AssertionError(
                    f"activation cache not implemented for {type(self.pipe)}"
                )

            self.activation_cache_handler = ActivationCacheHandler(
                activation_cache, positions_to_cache
            )
            # register hooks created by activation_cache
            self._set_hooks(
                position_hook_dict=self.activation_cache_handler.forward_hooks_dict,
                with_kwargs=True,
            )
        ...
        if use_qkv_cache:
            if isinstance(self.pipe, EditedFluxPipeline) or isinstance(
                ...
                self.qkv_cache_handler = QKVCacheFluxHandler(
                    self.pipe,
                    positions_to_cache,
    ...
    def activation_cache(self) -> ModelActivationCache:
        return (
            self.activation_cache_handler.cache
            if hasattr(self, "activation_cache_handler")
            and self.activation_cache_handler
            else None
        )
    ...
    def qkv_cache(self) -> QKVCache:
        return (
            self.qkv_cache_handler.cache
            if hasattr(self, "qkv_cache_handler") and self.qkv_cache_handler
            else None
        )
    ...
    def run(
        ...
        **kwargs,
        ...
        self.setup_cache(
            cache_activations, cache_qkv, positions_to_cache, inject_kv_mode=None
        )
        ...
        gen = [
            torch.Generator(device="cpu").manual_seed(seed) for _ in range(len(prompt))
        ]

        if inverse:
            # maybe create scheduler for inversion
            if not hasattr(self, "inversion_scheduler"):
                self.inversion_scheduler = (
                    FlowMatchEulerDiscreteSchedulerForInversion.from_config(
                        self.pipe.scheduler.config, inverse=True
                    )
                )
    ...
    def clear_all_hooks(self):
        ...
        for i in range(len(locate_block(self.pipe, "transformer.transformer_blocks"))):
            locate_block(
                self.pipe, f"transformer.transformer_blocks.{i}"
            )._forward_hooks.clear()

        for i in range(
            len(locate_block(self.pipe, "transformer.single_transformer_blocks"))
        ):
            locate_block(
                self.pipe, f"transformer.single_transformer_blocks.{i}"
            )._forward_hooks.clear()
    ...
    def _set_hooks(
        ...
        # Register hooks
        for is_pre_hook, hook_dict in [
            (True, position_pre_hook_dict),
            (False, position_hook_dict),
        ]:
                ...
                for h in hook:
                    self.registered_hooks.append(
                        register_general_hook(
                            self.pipe, position, h, with_kwargs, is_pre_hook
                        )
                    )
    ...
    def run_with_edit(
        ...
        **kwargs,
        ...
        edit_fn_hooks = {
            f"transformer.transformer_blocks.{layer}": [
                lambda *args: edit_streams_hook(
                    *args, recompute_fn=partial(edit_fn, layer=layer), stream=stream
                )
            ]
            for layer in layers_for_edit_fn
            if layer < 19
        }
        edit_fn_hooks.update(
            {
                f"transformer.single_transformer_blocks.{layer - 19}": [
                    lambda *args: edit_streams_hook(
                        *args, recompute_fn=partial(edit_fn, layer=layer), stream=stream
                    )
                ]
