...
def inference_for_example(example, cached_pipe, settings_dict):
        ...
        [
            ...
            torch.where(
                ...
                * torch.randn_like(example_noise["noise"]["foreground_noise"])
                + (1 - alpha_random) * example_noise["noise"]["foreground_noise"],
                example_noise["noise"]["background_noise"],
            ),
        ]
    )
    ...
    images = cached_pipe.run_inject_qkv(
        ...
        processor_class=partial(
            DitEditProcessor,
            call_max_times=int(
                settings_dict["tau_alpha"] * settings_dict["num_inference_steps"]
            ),
        ),
        width=512,
        height=512,
...
def inference_for_example_dict(
    ...
    for category, examples in example_dict.items():
        ...
        for example in examples:
            fg_restored, bg_restored, result_image = inference_for_example(
                example, cached_pipe, settings_dict
            )
            ...
            background_mse = compute_background_mse(
                example.bg_image, result_image, example.target_mask
            )
            clip_text_image = compute_clip_similarity(result_image, example.prompt)

            dinov2_similarity = compute_dinov2_similarity(
                result_image, example.fg_image, example.fg_mask
            )
...
def calculate_all_scores(
    ...
    columns = ["image_index", "model_type", "category"] + [
        ...
        if col not in ["image_index", "model_type", "category"]
    ]
...
def get_scores_for_single_example(
    example: BenchmarkExample, methods: list[str] = []
) -> dict:
    ...
    if example.result_image and "naive" in methods:  # "Photoshop Baseline"
        ...
        hpsv2_score_fg = compute_hpsv2_score(
            example.result_image, example.prompt, example.fg_mask
        )
        aesthetics_score = compute_aesthetics_score(example.result_image)
        aesthetics_score_fg = compute_aesthetics_score(
            example.result_image, example.fg_mask
        )
        background_mse = compute_background_mse(
            example.bg_image, example.result_image, example.target_mask
        )
        clip_text_image = compute_clip_similarity(example.result_image, example.prompt)
        clip_fg_image = compute_clip_similarity_images(
            example.result_image, example.fg_image, example.fg_mask
        )
        dinov2_similarity = compute_dinov2_similarity(
            example.result_image, example.fg_image, example.fg_mask
        )
        lpips = compute_lpips_score(
            example.result_image, example.fg_image, example.fg_mask
        )
    ...
    if example.tf_icon_image and "tf-icon" in methods:
        ...
        hpsv2_score_fg = compute_hpsv2_score(
            example.tf_icon_image, example.prompt, example.fg_mask
        )
        aesthetics_score = compute_aesthetics_score(example.tf_icon_image)
        aesthetics_score_fg = compute_aesthetics_score(
            example.tf_icon_image, example.fg_mask
        )
        background_mse = compute_background_mse(
            example.bg_image, example.tf_icon_image, example.target_mask
        )
        clip_text_image = compute_clip_similarity(example.tf_icon_image, example.prompt)
        clip_fg_image = compute_clip_similarity_images(
            example.tf_icon_image, example.fg_image, example.fg_mask
        )
    ...
    if example.kvedit_image and "kv-edit" in methods:
        ...
        hpsv2_score_fg = compute_hpsv2_score(
            example.kvedit_image, example.prompt, example.fg_mask
        )
        aesthetics_score = compute_aesthetics_score(example.kvedit_image)
        aesthetics_score_fg = compute_aesthetics_score(
            example.kvedit_image, example.fg_mask
        )
        background_mse = compute_background_mse(
            example.bg_image, example.kvedit_image, example.target_mask
        )
        clip_text_image = compute_clip_similarity(example.kvedit_image, example.prompt)
        clip_fg_image = compute_clip_similarity_images(
            example.kvedit_image, example.fg_image, example.fg_mask
        )
        # TODO: how to compute dinov2 score for our purposes?
        dinov2_similarity = compute_dinov2_similarity(
            example.kvedit_image, example.fg_image, example.fg_mask
    ...
    if example.output and "ours" in methods:
        ...
        hpsv2_score_fg = compute_hpsv2_score(
            example.output, example.prompt, example.fg_mask
        )
        ...
        background_mse = compute_background_mse(
            example.bg_image, example.output, example.target_mask
        )
        f_text_image = compute_clip_similarity(example.output, example.prompt)
        clip_fg_image = compute_clip_similarity_images(
            example.output, example.fg_image, example.fg_mask
        )
        dinov2_similarity = compute_dinov2_similarity(
            example.output, example.fg_image, example.fg_mask
        )
    ...
    if example.testing_image and "testing" in methods:
        ...
        hpsv2_score_fg = compute_hpsv2_score(
            example.testing_image, example.prompt, example.fg_mask
        )
        aesthetics_score = compute_aesthetics_score(example.testing_image)
        aesthetics_score_fg = compute_aesthetics_score(
            example.testing_image, example.fg_mask
        )
        background_mse = compute_background_mse(
            example.bg_image, example.testing_image, example.target_mask
        )
        clip_text_image = compute_clip_similarity(example.testing_image, example.prompt)
        clip_fg_image = compute_clip_similarity_images(
            example.testing_image, example.fg_image, example.fg_mask
        )
        dinov2_similarity = compute_dinov2_similarity(
            example.testing_image, example.fg_image, example.fg_mask
        )
...
def compute_background_mse(
    ...
    masked_diff = (
        diff_squared * include_mask[:, :, np.newaxis]
...
def compute_clip_similarity_images(
    image1: Image.Image, image2: Image.Image, mask
) -> float:
    """
    Compute CLIP similarity between a PIL image and a text prompt.
    Loads CLIP model only once and caches it.
...
def compute_hpsv2_score(composed_image: Image.Image, prompt, mask=None):
    ...
    from .hpsv2_open_clip_master.factory import (
    ...
    with torch.no_grad():
        ...
        # Process the image
        image = (
            compute_hpsv2_score.preprocess_val(composed_image)
            .unsqueeze(0)
            .to(device=device, non_blocking=True)
        )
        # Process the prompt
        text = compute_hpsv2_score.tokenizer([prompt]).to(
...
def compute_aesthetics_score(composed_image: Image.Image, mask=None):
    ...
    if mask is not None:
        ...
        # Crop the masked region and resize back to original size
        composed_image = composed_image.crop(bbox)
    pil_image = composed_image  # NOTE: put potential transformations here

    # preprocess image
    image = compute_aesthetics_score.preprocess(
    ...
    prediction = compute_aesthetics_score.model(
...
def compute_lpips_score(
    img1: Image.Image, img2: Image.Image, mask, net: str = "alex"
) -> float:
    """
