...
def masked_mse_tiled_mask(
    ...
    upsampled_mask = np.expand_dims(
        np.kron(tile_mask, np.ones((tile_size, tile_size), dtype=np.float32)), axis=-1
    )
...
def compute_dinov2_similarity(image1: Image.Image, image2: Image.Image) -> float:
    ...
    inputs = processor(
        images=[image1.convert("RGB"), image2.convert("RGB")], return_tensors="pt"
    )
