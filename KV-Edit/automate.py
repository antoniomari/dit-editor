...
class FluxEditor_kv_demo:
    ...
    def inverse(self, brush_canvas,
        ...
        self.init_image = self.encode(init_image, self.device[1]).to(self.device[0]) # Encode on device[1], move to device[0]
        print("Image encoded.")
        ...
        with torch.no_grad():
            ...
            inp = prepare(self.t5, self.clip, self.init_image.to(self.device[1]), prompt=opts.source_prompt) # Pass self.init_image to device[1] for prepare
    ...
    def edit(self, brush_canvas,
        ...
        with torch.no_grad():
            ...
            inp_target = prepare(self.t5, self.clip, self.init_image.to(self.device[1]), prompt=opts.target_prompt) # Pass self.init_image to device[1] for prepare
        ...
        self.ae.decoder.to(self.device[1])
...
if __name__ == "__main__":
    ...
    parser.add_argument("--input-dir", type=str, default="../benchmark_images_generations/Real-Real/", help="Base directory containing image sample subfolders")
    ...
    for sample_dir in sample_dirs:
        ...
        mask_images = glob(os.path.join(sample_dir, 'mask_bg_fg.jpg')) + \
            glob(os.path.join(sample_dir, 'mask_bg_fg.png'))
        ...
        if processed_count + skipped_count == 15:
            break
